...
# dependencies (rails, dwar, sqlite3, puma) regardless of cwd. An explicit
# BUNDLE_GEMFILE (e.g. a gemfiles/rails_*.gemfile matrix cell) is respected;
# only a pointer at the dummy's own partial Gemfile is reset to the root, so
# a stale BUNDLE_GEMFILE pointing there cannot leave the dummy on a partial
# bundle. Other missing-file cases are left to bundler to report.
